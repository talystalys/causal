...
fn test_deliberate_write_syscall_capture() {
    ...
    // Verify SYS_write (nr=1 on x86_64) entry exists with fd=1 (STDOUT_FILENO) and count=6
    ...
    for (i, line) in lines.iter().enumerate() {
        if line.starts_with("syscall-enter") && line.contains("nr=1 ") {
            // Check arguments: arg0 is fd (1), arg1 is buffer address (>0), arg2 is count (6)
            assert!(
            ...
            // Search forward for the paired exit
...
fn test_getpid_syscall_return_value() {
    ...
    // Verify SYS_getpid (nr=39 on x86_64) entry and exit exist and return matching positive pid
    let mut found_getpid = false;
    let lines: Vec<&str> = stdout.lines().collect();

    for (i, line) in lines.iter().enumerate() {
        if line.starts_with("syscall-enter") && line.contains("nr=39 ") {
            // Find tid from line: "syscall-enter tid=1234 nr=39 ..."
...
fn test_sigtrap_not_confused_with_syscall_stop() {
    ...
    // SIGTRAP is signal 5; exit code should be 128 + 5 = 133
    assert_eq!(
        output.status.code(),
        Some(133),
